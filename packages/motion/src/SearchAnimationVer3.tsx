...
const CancelButton: React.FC<CancelButtonProps> = ({ isSearching, onClick }) => {
  return (
    <motion.button
      ...
      transition={{
        duration: 0.15,
        delay: isSearching ? 0.35 : 0, // 이동 완료 후 나타남
        ease: [0, 0, 1, 1],
      }}
      onClick={onClick}
    >
    ...
    <div style={{ 
      ...
      <div
        style={{
          ...
          padding: '10px 16px',
          paddingRight: '8px',
          width: `calc(100% - ${CANCEL_BUTTON_SPACE}px)`,
          zIndex: 2,
        }}
      >
        <svg 
      ...
      {/* 우측 슬라이딩 영역 (배경) - 아래에서 슬라이드 */}
      <motion.div
        initial={{ x: CANCEL_BUTTON_SPACE }}
        animate={{ x: isSearching ? 0 : CANCEL_BUTTON_SPACE }}
        transition={{
          duration: 0.35,
          ease: [0.4, 0, 0.2, 1],
        }}
